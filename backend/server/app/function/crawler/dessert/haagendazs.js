...
const run = async () => {
  ...
  await page.goto('https://www.members.haagen-dazs.com.tw/Portal/StoreList', { timeout: 30000, waitUntil: 'networkidle2' });

  const pageContent = await page.content();
  const $ = cheerio.load(pageContent);
  const data = [];

  $('ul.store-list li.store-item').each((i, e) => {
    const name = $(e).find('.store-name span').text();
    const address = $(e).find('.store-info .content').eq(0).text();
    const phone = $(e).find('.store-info .content').eq(1).text();
    data.push({
      name,
      address,
      phone,
      brand_group: 'Haagen-Dazs',
      category1: '餐飲',
      category2: '甜點店',
    });
  });
